...
### gov.cn `/zhengce/` 真实列表（诊断 + 链接规则）

- 列表页输出 **`[gov.cn zhengce list diagnostics]`**：HTTP status、encoding、Content-Type、`a` 总数、href 前 20、过滤后条数与前 10 条；无链接时明确 **`no detail links extracted`**。
- 链接：gov.cn + 路径含 `/zhengce/` + 正文后缀；**不强制** `content_`；支持 `//` 协议相对链接；排除 index/list 等入口。
- 详情：gov 判定为 **gov.cn 且路径含 `/zhengce/`**；标题/正文/时间/来源多选择器；字段过短时 **`[gov.cn zhengce detail skip]`** 打到 stderr。
...
- **脚本**：`scripts/test_create_chinese_crawler_task.py` 支持 `--source-code`；未指定时 `--mode rss` 默认 `chinanews_society_rss`，`--mode static` 默认 `local_static_demo`；`--source-id` 仍可用但打印告警。
- **reset_demo_data**：新增 `--reset-sequences`（SQLite 清 `sqlite_sequence` 相关项；PG/MySQL 打印说明）。
- **文档**：README / `docs/backend-api.md` 写明不推荐写死 id；后续平台搜索源命名示例：`tieba_search`、`bilibili_search` 等（本版本不实现采集）。
