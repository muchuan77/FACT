...
- 对外**版本能力总览**以根目录 **`README.md`（v1.0.0 → v1.5.x 按序编排）** 为准；本日志补充过程、排障与论文级决策。
- 每次迭代至少记录：
  - 本阶段目标
  - 新增/修改的核心模块
  - 新增 API 或数据结构变化
...
## 2026-05-04 v1.5.0 Scrapy + Playwright 动态适配器

- **目标**：将 v1.1 实验结论 **dynamic_page → Scrapy + Playwright** 落地到 `fact_crawler`，与 RSS/static 共用 **`run_task_once` → `OpinionItem` → 后端 `run-now`** 去重与入库。
- **实现**：`scrapy_playwright_dynamic` 包（`spider.py` / `run_spider.py` / `subprocess_runner.py`）；父进程 **`PYTHONIOENCODING=utf-8`**、**`PYTHONUTF8=1`**；stdin **UTF-8 JSON**、stdout **单行 ASCII JSON**、stderr 诊断；无卡片时蜘蛛打印 **`rendered_html_length` / `page_title` / `detected_text_blocks_count` / `detected_links_count` / `text_head_300`**。
- **演示**：`dynamic_demo/index.html`（JS 延迟注入 `.opinion-card`）；种子 **`local_dynamic_demo`**（默认 **enabled=false**）；测试脚本 **`--mode dynamic`**，对 `local_dynamic_demo` 自动 PATCH **enabled=true**。
- **后端**：`run-now` 合并 **dynamic_sources**（`source_type=dynamic` 且 `adapter_name=scrapy_playwright_dynamic`）。
- **下一步（v1.6.0）**：平台/站内搜索类舆情源与适配器（不在 v1.5.0 范围）。
